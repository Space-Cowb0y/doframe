...
class OrganizerApp:
    def __init__(self):
        ...
        self.logic = DofusLogic(self.config, self.keymaps)
        self.logic.i18n = self.i18n
        self.gui = OrganizerGUI(self)
        self.logic.set_error_callback(
            lambda msg: self.gui.root.after(0, self.gui.show_temporary_message, msg, "#e74c3c")
        )
        self.current_idx = 0
        self.hotkey_actions = {} 
        self.mouse_hotkeys = {}
    ...
    def get_vk(self, key_str):
        key_str = self.keymaps.normalize_key_name(key_str)
        mapping = {
    ...
    def is_hotkey_pressed(self, hk_str):
        ...
        parts = hk_str.split('+')
        for p in parts:
            token = self.keymaps.normalize_key_name(p)
    ...
    def background_listener(self):
        ...
        while True:
            if self.config.data.get("spam_click_active", False):
                try:
                    fg_hwnd = win32gui.GetForegroundWindow()
                    is_dofus_focused = any(acc["hwnd"] == fg_hwnd for acc in self.logic.all_accounts)
                    if is_dofus_focused and win32api.GetAsyncKeyState(win32con.VK_MBUTTON) < 0:
    ...
    def register_action(self, hk_str, func):
        ...
        main_scan = None
        for p in parts:
            p = self.keymaps.normalize_key_name(p)
    ...
    def setup_hotkeys(self):
        ...
        try:
            ...
            if cfg.get("sync_key"): self.register_action(cfg["sync_key"], self.logic.sync_click_all)
            if cfg.get("sync_right_key"): self.register_action(cfg["sync_right_key"], self.logic.sync_right_click_all)
            if cfg.get("treasure_key"): self.register_action(cfg["treasure_key"], self.logic.execute_treasure_hunt)
            if cfg.get("swap_xp_drop_key"): self.register_action(cfg["swap_xp_drop_key"], self.logic.execute_swap_xp_drop)
            if cfg.get("invite_group_key"): self.register_action(cfg["invite_group_key"], self.logic.execute_group_invite)
            if cfg.get("auto_zaap_key"): self.register_action(cfg["auto_zaap_key"], self.logic.execute_auto_zaap)
            if cfg.get("paste_enter_key"): self.register_action(cfg["paste_enter_key"], self.logic.execute_paste_enter)
            if cfg.get("sort_taskbar_key"): self.register_action(cfg["sort_taskbar_key"], lambda: self.gui.root.after(0, self.gui.trigger_sort_taskbar))
    ...
    def on_radial_focus_select(self, selected_name):
        cycle_list = self.logic.get_cycle_list()
        for index, acc in enumerate(cycle_list):
...
def run_as_admin():
    if getattr(sys, 'frozen', False):
        return ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, " ".join(sys.argv[1:]), None, 1) > 32
    else:
        script = os.path.abspath(sys.argv[0])
        params = ' '.join([f'"{arg}"' for arg in sys.argv[1:]])
        return ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, f'"{script}" {params}', None, 1) > 32
...
def start_application():
    if not is_admin():
        if not run_as_admin():
            ctypes.windll.user32.MessageBoxW(
                0,
                "DOFRAME needs administrator rights to use global keyboard hooks.",
                "DOFRAME startup cancelled",
                0x10,
            )
        sys.exit() 
    cfg = Config()       
...
if __name__ == "__main__":
    try:
        ...
        log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "doframe_error.log")
        with open(log_path, "w", encoding="utf-8") as f:
            f.write(traceback.format_exc())
        try:
            ctypes.windll.user32.MessageBoxW(
                0,
                f"DOFRAME crashed during startup.\n\nDetails were written to:\n{log_path}",
                "DOFRAME startup error",
                0x10,
            )
        except Exception:
            pass
